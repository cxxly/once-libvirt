...
from bnlibvirt.web.httpserver import HttpServer, UnixHttpServer

from bnlibvirt import BNVMAPI, BNOptions
from bnlibvirt.BNLogging import log
from bnlibvirt.BNClient import XEN_API_SOCKET
# from bnlibvirt.XendDomain import instance as xenddomain
from bnlibvirt.web.SrvDir import SrvDir

from SrvRoot import SrvRoot
from XMLRPCServer import XMLRPCServer

xoptions = BNOptions.instance()
...
def _loadConfig(servers, root, reload):
    ...
    if api_cfg:
        try:
            for server_cfg in api_cfg:
                ...
                if len(server_cfg) > 1:
                    if server_cfg[1] in [BNVMAPI.AUTH_PAM, BNVMAPI.AUTH_NONE]:
    ...
    if xoptions.get_xend_tcp_xmlrpc_server():
        ...
        if ssl_key_file and ssl_cert_file:
            servers.add(XMLRPCServer(BNVMAPI.AUTH_PAM, False, use_tcp = True,
                                     ssl_key_file = ssl_key_file,
                                     ssl_cert_file = ssl_cert_file,
                                     host = addr, port = port))
        else:
    ...
    if xoptions.get_xend_unix_xmlrpc_server():
        servers.add(XMLRPCServer(BNVMAPI.AUTH_PAM, False))
