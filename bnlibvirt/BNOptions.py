...
from bnlibvirt import sxp, osdep, BNLogging
from bnlibvirt.BNError import BNError
from bnlibvirt.util import auxbin
...
class XendOptions:
    ...
    logfile_default = "/var/log/xen/libvirt.log"
    ...
    def configure(self):
        self.set_config()
        BNLogging.init(self.get_config_string("logfile",
    ...
    def get_xend_tcp_xmlrpc_server_ssl_key_file(self):
        ...
        if file and not os.path.exists(file):
            raise BNError("invalid xend config %s: directory '%s' does not exist" % (name, file))
        return file
    ...
    def get_vnc_x509_cert_dir(self):
        ...
        if not os.path.exists(vncdir):
            raise BNError("invalid xend config %s: directory '%s' does not exist" % (name, vncdir))
        return vncdir
...
class XendOptionsFile(XendOptions):
    ...
    def get_config_bool(self, name, val=None):
        ...
        if v in ['no',  'n', '0', 'off', 'false', 'f']:
            return False
        raise BNError("invalid xend config %s: expected bool: %s" % (name, v))
    ...
    def get_config_int(self, name, val=None):
        ...
        try:
            ...
            raise BNError("invalid xend config %s: expected int: %s" % (name, v))
